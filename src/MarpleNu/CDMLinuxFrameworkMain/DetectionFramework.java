...
import MarpleNu.CDMDataParser.EventParser;
    ...
    public void addEvent(EventForCS event){
        ...
        if(event.eventType == 2||event.eventType==3||event.eventType==13){
            ...
            if(event.parentUuid != event.uuid){
                ...
                transLabel(parentNode, newNode, event.eventType, transGrammar, 0);
            }
        }
...
//            }
    ...
    public void addLabel(LabelForCS l){
        ...
        if(l.pid == -1){
            ...
            ProcessNode Node = processList.get(l.uuid);
            if(Node == null){
                Node = new ProcessNode(l.pid, l.uuid);
                Node.pcid = l.pcid;
                processList.put(l.uuid, Node);
                Node.parentNode = processRoot;
                processRoot.childNode.add(Node);
            }
    ...
    private int judgePHF(String label){
    ...
    public void rewrite_to_file(String s) {
        try {
            File f1 = new File("src/IPFile.out");
            if (f1.exists() == false) {
                f1.getParentFile().mkdirs();
            }
            String s1 = s + "\n";
            FileWriter writer = new FileWriter("src/IPFile.out", true);
            writer.write(s1);
            writer.close();
    ...
    private void judgeOutput(Node node){
        ...
        for(String judgeMode : judgeGrammar.keySet()){
            ...
            if(sum == judge.length){
                ...
                ProcessNode pppp=(ProcessNode)node;
                if(pppp.pid==33253)
                {
                    int qqqqqqqqqqq=1;
                }
...
//        }
    ...
    private void outputProcess(ProcessNode processRoot, String time)throws IOException{
        ...
        File file1 = new File(String.format(String.format("dot_%s_", clientId) + time + "_Process.dot"));
        if (!file1.getParentFile().exists()){
            file1.getParentFile().mkdirs();
        }
        FileWriter writer = new FileWriter(file1, false);
    ...
    private void outputProcessDotAndFileCsv() {
        ...
        try {
            for(String pcid : ProcessList.keySet()){
                ...
                outputFile(FileList.get(pcid), pcid + "_" + currentTime);
                outputProcess(ProcessRoot.get(pcid), pcid + "_" + currentTime);
                outputProcessList(ProcessList.get(pcid), pcid + "_" + currentTime);
                outputProcessList1(ProcessList.get(pcid), pcid + "_" + currentTime);
            }
